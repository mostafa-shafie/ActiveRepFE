...
import { Phone, Calendar, Users, BarChart, PlaneLanding, Bell, LayoutDashboard, Star } from "lucide-react"
...
export default function Component() {
  ...
  const novaScoreData = {
    ...
    changeColor: "#27AE60",
    insights: "Your NOVA-Score is improving! Keep focusing on key behaviors to reach the target.",
    highlighted: true
  }
  ...
  const cardData = [
    ...
    },
    {
      ...
      change: "-5% from last month",
      changeColor: "#E74C3C",
      insights: "Your frequency achievement has decreased. Focus on meeting your planned visit frequency."
    },
    {
      title: "Coverage",
      ...
      changeColor: "#27AE60",
      insights: "Great job improving your coverage! You're reaching more customers effectively."
    },
    {
      ...
      change: "-3% from last month",
      changeColor: "#E74C3C",
      insights: "Your actual calls are slightly below planned. Try to stick closer to your schedule for better results."
    }
  ]
      ...
      <main className="flex-1 relative z-10">
        <section className="w-full py-12 md:py-24 lg:py-32 xl:py-48">
          <div className="container px-4 md:px-6">
            ...
            >
              <div className="space-y-2">
                ...
                <p className="mx-auto max-w-[700px] text-[#2C3E50] md:text-xl">
                  Monitor your key performance indicators and plan your customer interactions effectively.
                </p>
              </div>
            </motion.div>
            <motion.div 
            ...
            >
              <Card 
                className="bg-white/90 border-[#BDC3C7] backdrop-blur-sm ring-2 ring-[#F1C40F] shadow-lg"
              >
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-xl font-bold text-[#2C3E50]">{novaScoreData.title}</CardTitle>
                  {novaScoreData.icon}
                </CardHeader>
                <CardContent>
                  <div className="flex items-baseline">
                    <div className="text-4xl font-bold text-[#1F3A93]">{novaScoreData.value}%</div>
                    <div className="ml-2 text-sm font-medium" style={{ color: novaScoreData.changeColor }}>{novaScoreData.change}</div>
                  </div>
                  <div className="mt-4">
                    <div className="flex justify-between text-sm text-[#2C3E50] mb-1">
                      <span>Progress</span>
                      <span>{Math.round((novaScoreData.value / novaScoreData.target) * 100)}%</span>
                    </div>
                    <div className="w-full bg-[#BDC3C7] rounded-full h-3">
                      <div 
                        className="bg-[#F1C40F] h-3 rounded-full"
                        style={{ width: `${(novaScoreData.value / novaScoreData.target) * 100}%` }}
                      ></div>
                    </div>
                    <p className="text-sm text-[#2C3E50] mt-2">Target: {novaScoreData.target}%</p>
                  </div>
                  <p className="mt-4 text-sm text-[#2C3E50]">{novaScoreData.insights}</p>
                </CardContent>
              </Card>
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                {cardData.map((card, index) => (
                  ...
                  >
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium text-[#2C3E50]">{card.title}</CardTitle>
                      {card.icon}
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold text-[#1F3A93]">{card.value}{card.title.includes('%') ? '%' : ''}</div>
                      <p className="text-xs" style={{ color: card.changeColor }}>{card.change}</p>
                      <div className="mt-4">
                        ...
                        <p className="text-xs text-[#2C3E50] mt-1">Target: {card.target}{card.title.includes('%') ? '%' : ''}</p>
                      </div>
                    </CardContent>
                  </Card>
                ))}
            ...
            >
              <Button asChild className="bg-[#1F3A93] hover:bg-[#3498DB] text-white">
                <Link href="https://b_Pk8N3DvBuXR.v0.build/">Go to Planner</Link>
              </Button>
